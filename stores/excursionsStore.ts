// Store de excursiones: catálogo, filtros y búsqueda (con caché offline persist)
...
export const useExcursionsStore = create<ExcursionsState>()(
  persist(
    (set, get) => ({
      excursions: [],
      ...
      // Si falla la red, intenta buscar en la caché persistida (excursions[]).
      getExcursionById: async (id: number) => {
        ...
        try {
          ...
          const cached = get().excursions.find((e) => e.id_excursion === id) ?? null;
          if (cached) {
            set({ currentExcursion: cached, loading: false });
            return cached;
          }
